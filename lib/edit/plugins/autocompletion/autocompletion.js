	...
	,_checkLetter: function(){
		...
		if( editArea.is_editable )
		{
			...
			else
			{
				...
				// sort results
				this.container.innerHTML		= '<ul class="autocomplete">'+ lines.sort().join('') +'</ul>';
				
				var cursor	= _$("cursor_pos");
				this.container.style.top		= ( ( cursor.cursor_top + editArea.lineHeight ) + 53 ) +"px";
				this.container.style.left		= ( ( cursor.cursor_left + 8 ) - 24 ) +"px";
				this._show();
